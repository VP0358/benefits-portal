...
const STATUS_CONFIG: Record<string, StatusConfig> = {
  ...
  pending:         { emoji: "🌱", label: "登録中",       borderColor: "#84cc16", bgColor: "rgba(132,204,22,0.07)",   textColor: "#65a30d", badgeBg: "rgba(132,204,22,0.14)",  badgeText: "#4d7c0f" },
  provisional:     { emoji: "👤", label: "仮登録",       borderColor: "#94a3b8", bgColor: "rgba(148,163,184,0.07)", textColor: "#475569", badgeBg: "rgba(148,163,184,0.14)", badgeText: "#475569" },
  waiting_payment: { emoji: "⏳", label: "入金待ち",     borderColor: "#f59e0b", bgColor: "rgba(245,158,11,0.07)",   textColor: "#b45309", badgeBg: "rgba(245,158,11,0.14)",  badgeText: "#b45309" },
  canceled:        { emoji: "❌", label: "キャンセル",   borderColor: "#ef4444", bgColor: "rgba(239,68,68,0.07)",    textColor: "#b91c1c", badgeBg: "rgba(239,68,68,0.14)",   badgeText: "#b91c1c" },
  fan:             { emoji: "💚", label: "愛用会員",     borderColor: "#10b981", bgColor: "rgba(16,185,129,0.07)",   textColor: "#047857", badgeBg: "rgba(16,185,129,0.14)",  badgeText: "#047857" },
  lapsed:          { emoji: "💀", label: "失効",         borderColor: "#9ca3af", bgColor: "rgba(156,163,175,0.07)", textColor: "#6b7280", badgeBg: "rgba(156,163,175,0.14)", badgeText: "#4b5563" },
  ...
  mid_cancel:      { emoji: "🚪", label: "中途解約",     borderColor: "#f97316", bgColor: "rgba(249,115,22,0.07)",   textColor: "#c2410c", badgeBg: "rgba(249,115,22,0.14)",  badgeText: "#c2410c" },
  ...
  cooling_off:     { emoji: "🔄", label: "クーリングオフ", borderColor: "#0ea5e9", bgColor: "rgba(14,165,233,0.07)", textColor: "#0369a1", badgeBg: "rgba(14,165,233,0.14)",  badgeText: "#0369a1" },
};
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center"
      ...
      <div className="w-full sm:max-w-sm rounded-t-3xl sm:rounded-3xl overflow-hidden"
          ...
          <div className="flex items-start justify-between gap-3">
            <div className="flex items-center gap-3">
              <div className="w-12 h-12 rounded-full flex items-center justify-center text-2xl flex-shrink-0"
                style={{ background: sc.bgColor, border: `2px solid ${sc.borderColor}50` }}>
                {node.avatarUrl
        ...
        <div className="overflow-y-auto px-5 py-3 space-y-0" style={{ maxHeight: "55vh" }}>
        ...
        <div className="px-5 pb-5 pt-3" style={{ borderTop: "1px solid #f1f5f9" }}>
          <button onClick={onClose} className="w-full py-2.5 rounded-xl text-sm font-bold transition"
...
function MemberCard({
  ...
  const codeColor = isRoot ? GOLD_LIGHT : sc.textColor;
  const ptColor   = isRoot ? GOLD_LIGHT : "#7c3aed";
  const subColor  = isRoot ? "rgba(255,255,255,0.50)" : "#64748b";

  return (
    <div
      className="relative rounded-xl overflow-visible flex-shrink-0 cursor-pointer hover:opacity-90 transition-opacity"
      style={{
        width: "200px",
        background: cardBg,
        border: cardBorder,
    ...
    >
      ...
      <div className="absolute -top-3 -left-2 z-10 rounded-full flex items-center justify-center font-black"
        style={{
          width: "30px", height: "20px", fontSize: "9px",
      ...
      <div className="absolute -top-3 -right-2 z-10 rounded-full flex items-center justify-center font-bold"
        style={{
          width: "26px", height: "20px", fontSize: "9px",
          background: isRoot ? `linear-gradient(135deg,${GOLD},${ORANGE})` : NAVY_CARD3,
          color: isRoot ? "#fff" : GOLD_LIGHT,
          border: isRoot ? `1px solid ${GOLD_DARK}` : `1px solid ${GOLD}40`,
      ...
      <div className="flex items-stretch gap-0 px-2.5 pt-4 pb-2.5">
        ...
        <div className="flex-shrink-0 flex flex-col items-center justify-center mr-2.5">
          <div
            ...
            style={{
              ...
              background: isRoot ? NAVY_CARD3 : `${sc.borderColor}18`,
              border: isRoot ? `2px solid ${GOLD}50` : `2px solid ${sc.borderColor}50`,
            }}
          >
            {node.avatarUrl
        ...
        <div className="flex-1 min-w-0 flex flex-col gap-0.5">
          ...
          <div className="font-mono font-bold leading-none"
            style={{ fontSize: "11px", color: codeColor, letterSpacing: "0.02em" }}>
            {node.mlmMemberCode}
          </div>

          {/* 名前 */}
          <div className="font-bold leading-tight truncate"
            style={{ fontSize: "13px", color: nameColor, maxWidth: "130px" }}>
            {node.name.length > 10 ? node.name.slice(0, 9) + "…" : node.name}
          </div>

          {/* ステータスバッジ */}
          <div>
            <span className="inline-block font-bold rounded-full px-1.5 py-0.5 leading-none"
          ...
          <div className="flex items-center gap-2 mt-0.5">
            <span className="font-black leading-none" style={{ fontSize: "12px", color: ptColor }}>
              {node.selfPoints.toLocaleString()}pt
            </span>
            {node.totalDescendants > 0 && (
              <span className="font-bold leading-none" style={{ fontSize: "11px", color: subColor }}>
...
// ── ① ビジュアルツリーノード ─────────────────────────────────
// ★ 修正: minDepth廃止。常にdepth=1から表示し、maxDepthで打ち切り（累積表示）
function VisualTreeNode({
  ...
  node: NodeData;
  depth: number;
  maxDepth: number;  // この段まで展開して表示（累積: 常に1〜maxDepth）
  ...
  // maxDepthを超えた段は「続きあり」表示（クリック不可の情報バッジ）
  if (depth > maxDepth) {
    return (
      <div className="flex flex-col items-center" style={{ minWidth: "80px" }}>
        ...
        <div className="rounded-lg px-2 py-1 text-center"
          ...
          <div style={{ fontSize: "9px", color: GOLD, fontWeight: 700 }}>
            {node.name.length > 6 ? node.name.slice(0, 5) + "…" : node.name}
          </div>
          {node.totalDescendants > 0 && (
            <div style={{ fontSize: "8px", color: `${GOLD}70` }}>
              傘下{node.totalDescendants}名
            </div>
          )}
        </div>
  ...
  return (
    <div className="flex flex-col items-center" style={{ minWidth: "210px" }}>
      {/* カード */}
      ...
      {!hasChildren && <div className="h-3" />}

      {/* 子ノード群（横展開） */}
      {expanded && hasChildren && (
        <div className="flex flex-row items-start" style={{ position: "relative" }}>
          {/* 水平接続線 */}
          {children.length > 1 && (
          ...
          {children.map(child => (
            <div key={child.id} className="flex flex-col items-center" style={{ padding: "0 5px", position: "relative" }}>
  ...
  rows.push(
    <div key={node.id}
      ...
      <div className="w-7 h-7 rounded-full overflow-hidden flex-shrink-0 flex items-center justify-center text-sm"
        ...
        {node.avatarUrl ? <img src={node.avatarUrl} alt={node.name} className="w-full h-full object-cover" /> : <span>{sc.emoji}</span>}
      </div>
      ...
      <div className="flex items-center gap-1.5 flex-shrink-0">
        ...
        <span className="font-bold rounded-full px-1.5 py-0.5"
          style={{ fontSize: "10px", background: sc.badgeBg, color: sc.badgeText }}>{sc.emoji}</span>
      </div>
    </div>
  );
  if (depth < maxDepth) {
    for (const child of node.children) {
      rows.push(<ListRow key={`${child.id}-r`} node={child} depth={depth + 1} orgType={orgType} maxDepth={maxDepth} onNodeClick={onNodeClick} />);
    }
  } else if (node.children.length > 0) {
    rows.push(
      <div key={`${node.id}-more`} className="py-1 text-center"
        style={{ paddingLeft: `${8 + (depth + 1) * 14}px`, fontSize: "9px", color: `${GOLD}50` }}>
...
function DepthStatsModal({ depthStats, totalCount, activeCount, onClose }: {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center"
      ...
      <div className="w-full sm:max-w-sm rounded-t-3xl sm:rounded-3xl overflow-hidden"
        style={{ background: `linear-gradient(160deg,${NAVY} 0%,${NAVY_CARD} 30%,${NAVY_CARD2} 100%)`, border: `1px solid ${GOLD}40`, maxHeight: "85vh" }}
        onClick={e => e.stopPropagation()}>
        ...
        <div className="px-5 py-3 grid grid-cols-2 gap-3" style={{ borderBottom: `1px solid rgba(255,255,255,0.06)` }}>
          {[{ label: "配下合計", value: totalCount, color: GOLD_LIGHT }, { label: "アクティブ計", value: activeCount, color: "#22c55e" }].map(item => (
          ...
          {upTo7.map(stat => (
            <div key={stat.depth} className="rounded-xl px-4 py-2.5 flex items-center gap-3"
              ...
              <div className="w-10 h-8 rounded-lg flex items-center justify-center font-black text-xs flex-shrink-0"
                style={{ background: `${GOLD}18`, color: GOLD_LIGHT, border: `1px solid ${GOLD}35` }}>M{stat.depth}段</div>
          ...
          {beyond8.length > 0 && (
            <div className="rounded-xl px-4 py-2.5 flex items-center gap-3"
              ...
              <div className="w-10 h-8 rounded-lg flex items-center justify-center font-black flex-shrink-0"
                style={{ fontSize: "10px", background: `${GOLD}12`, color: GOLD, border: `1px solid ${GOLD}25` }}>8段+</div>
              <div className="flex-1">
                <div className="flex items-center justify-between mb-1">
  ...
  return (
    <div className="flex flex-col items-center">
      <div className="relative rounded-2xl overflow-visible"
        ...
        <div className="absolute -top-3 -left-2 z-10 rounded-full flex items-center justify-center font-black"
          style={{ width: "32px", height: "22px", fontSize: "9px", background: lc.bg, color: lc.text, border: `1.5px solid ${lc.border}`, boxShadow: "0 1px 4px rgba(0,0,0,0.25)" }}>
          LV.{me.currentLevel}
        </div>
        <div className="absolute -top-3 -right-2 z-10 rounded-full flex items-center justify-center font-black"
          style={{ width: "32px", height: "22px", fontSize: "9px", background: `linear-gradient(135deg,${GOLD},${ORANGE})`, color: "#fff", boxShadow: "0 1px 4px rgba(0,0,0,0.25)" }}>
          YOU
        </div>
        <div className="flex items-center gap-3 px-3 pt-5 pb-3">
          <div className="rounded-full overflow-hidden flex items-center justify-center text-2xl flex-shrink-0"
            style={{ width: "48px", height: "48px", background: NAVY_CARD3, border: `2px solid ${GOLD}50`, boxShadow: `0 0 12px ${GOLD}20` }}>
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-2 mt-1">
              <span className="font-bold rounded-full px-1.5 py-0.5"
                style={{ fontSize: "10px", background: me.isActive ? "rgba(34,197,94,0.18)" : "rgba(239,68,68,0.12)", color: me.isActive ? "#22c55e" : "#f87171" }}>
...
function LegendPanel() {
  return (
    <div className="mx-4 rounded-2xl px-4 py-3"
      ...
      {/* ステータス凡例 */}
      <p className="font-semibold mb-1.5" style={{ fontSize: "11px", color: `${NAVY}70` }}>凡例 — ステータス</p>
      <div className="flex flex-wrap gap-1">
        {Object.entries(STATUS_CONFIG).map(([key, sc]) => (
          <span key={key} className="font-bold rounded-full px-1.5 py-0.5"
            style={{ fontSize: "9px", background: sc.badgeBg, color: sc.badgeText, border: `1px solid ${sc.borderColor}35` }}>
            {sc.emoji} {sc.label}
          </span>
        ))}
      </div>
      {/* レベル凡例 */}
      <p className="font-semibold mt-2 mb-1" style={{ fontSize: "11px", color: `${NAVY}70` }}>レベル</p>
      <div className="flex flex-wrap gap-1">
        {[0, 1, 2, 3, 4, 5].map(lv => {
          const lc = getLevelColor(lv);
          return (
            <span key={lv} className="font-bold rounded-full px-2 py-0.5"
              style={{ fontSize: "9px", background: lc.bg, color: lc.text, border: `1px solid ${lc.border}`, boxShadow: "0 1px 3px rgba(0,0,0,0.12)" }}>
              LV.{lv}
            </span>
          );
        })}
      ...
      <div className="mt-1.5 flex items-center gap-1" style={{ fontSize: "10px" }}>
        <span>⚠️</span>
        <span className="font-semibold" style={{ color: "#b45309" }}>要注意（4ヶ月）/ 失効（5ヶ月）</span>
        <span style={{ color: `${NAVY}45` }}>— スミサイ連続未購入</span>
      </div>
    </div>
  );
}
...
export default function MlmOrgChartPage() {
  ...
  // ① ページング: 常に1段目から表示し、maxDepthを5ずつ増やす（累積表示）
  const [maxDepthPage, setMaxDepthPage] = useState(0); // 0→1〜5段, 1→1〜10段, ...
  ...
  const fetchData = useCallback(() => {
    ...
    fetch("/api/my/mlm-org-chart")
      .then(r => { if (!r.ok) throw new Error(r.status === 404 ? "MLM会員情報がありません" : "取得失敗"); return r.json(); })
      .then(setData)
  ...
  const STEP = 5;
  // ① 累積maxDepth: 常にdepth=1〜effectiveMaxDepth を表示
  const effectiveMaxDepth = isMatrix ? (maxDepthPage + 1) * STEP : 7;

  // 最大段数（depthStatsから算出）
  const maxAvailableDepth = depthStats.length > 0 ? Math.max(...depthStats.map(s => s.depth)) : STEP;
  const canGoNext = isMatrix && effectiveMaxDepth < maxAvailableDepth;
  const canGoPrev = isMatrix && maxDepthPage > 0;

  // ③ グループアクティブポイント（depthStatsのactive合計 × 仮ポイント — 実際はAPIから取得が理想だが統計で代替）
  // ここでは totalCount/activeCount をサマリーカードに表示済みのため、
  // ヘッダーサマリーに「段別内訳」ボタンで代替。
  ...
  return (
    <div className="min-h-screen pb-20" style={{ background: PAGE_BG }}>
        ...
        <div className="max-w-full mx-auto px-4 pb-2 flex items-center justify-between gap-3">
          <div className="flex rounded-xl overflow-hidden border" style={{ borderColor: `${GOLD}25` }}>
            {([{ key: "matrix", label: "🔷 マトリックス" }, { key: "unilevel", label: "🔶 ユニレベル" }] as { key: OrgType; label: string }[]).map(({ key, label }) => (
              <button key={key} onClick={() => { setOrgType(key); setMaxDepthPage(0); }}
                className="px-3 py-1.5 text-xs font-bold transition"
                style={{ background: orgType === key ? `linear-gradient(135deg,${GOLD},${ORANGE})` : "rgba(10,22,40,0.04)", color: orgType === key ? "#fff" : `${NAVY}70` }}>
                {label}
              </button>
            ))}
          </div>
          <div className="flex rounded-xl overflow-hidden border" style={{ borderColor: `${GOLD}25` }}>
            {([{ key: "tree", label: "🌲 ツリー" }, { key: "list", label: "📋 リスト" }] as { key: ViewMode; label: string }[]).map(({ key, label }) => (
              <button key={key} onClick={() => setViewMode(key)}
                className="px-3 py-1.5 text-xs font-bold transition"
                style={{ background: viewMode === key ? "linear-gradient(135deg,#334155,#1e293b)" : "rgba(10,22,40,0.04)", color: viewMode === key ? "#e2e8f0" : `${NAVY}70` }}>
                {label}
              </button>
            ))}
          </div>
      ...
      <main className="py-4 space-y-4 relative">
        ...
        {error && !loading && (
          <div className="mx-4 rounded-2xl p-6 text-center" style={{ background: "rgba(239,68,68,0.06)", border: "1px solid rgba(239,68,68,0.20)" }}>
          ...
          <>
            ...
            <div className="px-4 grid grid-cols-2 gap-3">
              <div className="rounded-2xl p-4 text-center"
                ...
                <div className="font-bold" style={{ fontSize: "11px", color: `${GOLD}70` }}>👥 {isMatrix ? "配下合計" : "紹介合計"}</div>
              ...
              <button onClick={() => isMatrix && setShowDepthModal(true)}
                ...
                style={{ background: "rgba(34,197,94,0.10)", border: `1px solid rgba(34,197,94,0.28)`, boxShadow: "0 4px 12px rgba(10,22,40,0.10)", cursor: isMatrix ? "pointer" : "default" }}>
            ...
            {isMatrix && (
              <div className="px-4 flex items-center justify-between">
                <span className="font-bold" style={{ fontSize: "12px", color: `${GOLD}70` }}>
                  表示: 1〜{effectiveMaxDepth}段
                </span>
                <div className="flex gap-2">
                  {canGoPrev && (
                    <button onClick={() => setMaxDepthPage(p => p - 1)}
                      className="font-bold transition rounded-lg px-3 py-1"
                      style={{ fontSize: "12px", background: "rgba(255,255,255,0.70)", color: NAVY, border: `1px solid ${GOLD}25` }}>
                      ▲ 前の5段
                    </button>
                  )}
                  {canGoNext && (
                    <button onClick={() => setMaxDepthPage(p => p + 1)}
                      className="font-bold transition rounded-lg px-3 py-1"
                      style={{ fontSize: "12px", background: `${GOLD}20`, color: GOLD_DARK, border: `1px solid ${GOLD}35` }}>
                      ▼ 次の5段
                    </button>
                  )}
                </div>
            ...
            {viewMode === "tree" && (
              <div className="px-4 flex items-center justify-end gap-2">
                <span className="font-bold" style={{ fontSize: "11px", color: `${GOLD}60` }}>倍率: {Math.round(zoom * 100)}%</span>
                <div className="flex items-center rounded-xl overflow-hidden border" style={{ borderColor: `${GOLD}25` }}>
                  <button onClick={() => setZoom(z => Math.max(0.3, parseFloat((z - 0.1).toFixed(1))))}
                    className="px-2.5 py-1.5 text-sm font-black transition" style={{ background: "rgba(10,22,40,0.04)", color: NAVY }}>−</button>
                  <button onClick={() => setZoom(1.0)}
                    className="px-2.5 py-1 font-bold transition border-x"
                    style={{ fontSize: "10px", background: "rgba(10,22,40,0.02)", color: `${NAVY}70`, borderColor: `${GOLD}20` }}>FIT</button>
            ...
            {currentNodes.length === 0 ? (
              <div className="mx-4 rounded-2xl p-10 text-center" style={{ background: `${NAVY_CARD}80`, border: `2px dashed ${GOLD}15` }}>
              ...
              /* ─── ① ツリービュー（累積maxDepth、常にdepth=1から表示） ─── */
              <div ref={treeRef} className="overflow-x-auto overflow-y-visible pb-6"
                ...
                <div className="flex flex-col items-center"
                  ...
                  {/* 配下ツリー（常にdepth=1から・effectiveMaxDepthまで） */}
                  <div className="flex flex-row items-start" style={{ position: "relative" }}>
                    {currentNodes.length > 1 && (
                      <div className="absolute" style={{
                        top: 0,
                        left: `${100 / currentNodes.length / 2}%`,
                        right: `${100 / currentNodes.length / 2}%`,
                        height: "1px",
                        background: `${GOLD}35`,
                      }} />
                    )}
              ...
              <div className="mx-4 rounded-2xl overflow-hidden" style={{ background: NAVY_CARD, border: `1px solid ${GOLD}18` }}>
                ...
                {currentNodes.map(node => (
                  <ListRow key={node.id} node={node} depth={1} orgType={orgType} maxDepth={effectiveMaxDepth} onNodeClick={setSelectedNode} />
                ))}
              </div>
            )}
      ...
      {showDepthModal && data && (
        <DepthStatsModal depthStats={depthStats} totalCount={totalCount} activeCount={activeCount} onClose={() => setShowDepthModal(false)} />
      )}
    </div>
  );
}
