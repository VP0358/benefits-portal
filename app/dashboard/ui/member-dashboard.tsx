    ...
    <div className="min-h-screen bg-[#e6f2dc] pb-28 relative">
          ...
          >
            ...
            <nav className="flex flex-col gap-1 flex-1 px-3 py-3 overflow-y-auto">
              {[
                ...
                <Link
                  ...
                  className="px-4 py-3 rounded-xl text-sm text-gray-700 hover:bg-green-50 transition"
                >
                  {item.label}
                </Link>
              ))}
            ...
            <div className="px-3 pb-6">
              <button
                ...
                className="w-full px-4 py-3 rounded-xl text-sm text-red-500 hover:bg-red-50 transition text-left"
              >
                🚪 ログアウト
              </button>
            </div>
      ...
      <main className="max-w-md mx-auto px-4 pt-5 space-y-5">
        ...
        >
          ...
          <p className="text-xs opacity-75 mt-1">会員コード：{user.memberCode}</p>
        ...
        {/* ── お知らせスライダー ── */}
        <section id="news">
          <h2 className="text-sm font-bold text-gray-600 mb-2 px-1">📢 お知らせ</h2>
          {announcements.length === 0 ? (
            <div className="bg-white rounded-2xl p-5 text-center text-gray-400 text-sm shadow">
              現在お知らせはありません
            </div>
          ) : (
            <div className="bg-white rounded-2xl shadow overflow-hidden">
              <div className="p-4 min-h-[110px]">
                <div className="flex items-center gap-2 mb-2">
                  <span className={`text-xs px-2 py-0.5 rounded-full font-semibold ${TAG_STYLE[activeAnn?.tag] ?? "bg-gray-100 text-gray-600"}`}>
                ...
                <p className="font-semibold text-gray-800 text-sm">{activeAnn?.title}</p>
                <p className="text-xs text-gray-500 mt-1 line-clamp-3">{activeAnn?.content}</p>
              </div>
        ...
        {announcements.length > 0 && (
          <section>
            <h2 className="text-sm font-bold text-gray-600 mb-2 px-1">お知らせ一覧</h2>
            <div className="space-y-2">
              {announcements.map((a) => (
                <div key={a.id} className="bg-white rounded-2xl p-4 shadow flex gap-3 items-start">
                  <span className={`text-xs px-2 py-0.5 rounded-full font-semibold shrink-0 mt-0.5 ${TAG_STYLE[a.tag] ?? "bg-gray-100 text-gray-600"}`}>
        ...
        {/* ── 福利厚生メニュー ── */}
        <section id="menu">
          <h2 className="text-sm font-bold text-gray-600 mb-2 px-1">🛎️ 福利厚生メニュー</h2>
          <div className="grid grid-cols-3 gap-3">
            {menus.map((m) => (
              ...
              >
                <div className="text-2xl mb-1">{m.iconType ?? "📌"}</div>
                <p className="text-xs font-semibold text-gray-700 leading-tight">{m.title}</p>
                {m.subtitle && (
                  <p className="text-[10px] text-gray-400 mt-0.5">{m.subtitle}</p>
                )}
              </a>
            ))}
          </div>
        ...
        {/* ── クイックリンク ── */}
        <section>
          <h2 className="text-sm font-bold text-gray-600 mb-2 px-1">📌 クイックアクセス</h2>
      ...
      <nav className="fixed bottom-0 left-0 right-0 z-30 bg-white border-t border-gray-200 shadow-lg">
        <div className="max-w-md mx-auto flex items-end justify-around px-2 py-1">
          <Link href="/dashboard" className="flex flex-col items-center gap-0.5 py-2 px-3">
            ...
            <span className="text-[10px] text-gray-500">ホーム</span>
          </Link>
          <Link href="#menu" className="flex flex-col items-center gap-0.5 py-2 px-3">
            <span className="text-xl">📋</span>
            <span className="text-[10px] text-gray-500">メニュー</span>
          </Link>
          {/* 中央ポイントボタン（大きめ） */}
          <Link href="/points/use" className="flex flex-col items-center -mt-5">
            ...
            <span className="text-[10px] text-gray-500 mt-1">ポイント</span>
          </Link>
          <Link href="#news" className="flex flex-col items-center gap-0.5 py-2 px-3 relative">
            ...
            <span className="text-[10px] text-gray-500">お知らせ</span>
          </Link>
