      ...
      <div className="rounded-3xl bg-white shadow-sm overflow-hidden">
              ...
              {subs.map(s => {
                ...
                return (
                  <tr key={s.id.toString()} className="hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="px-5 py-3 font-medium text-slate-700">{s.planName}</td>
                    ...
                    <td className="px-5 py-3 text-xs text-slate-500">
                      {s.confirmedAt
                        ? new Date(s.confirmedAt).toLocaleDateString("ja-JP")
                        : "—"}
                    </td>
                    <td className="px-5 py-3 text-xs text-slate-500 max-w-[120px] truncate">
                      {s.note ?? "—"}
                    </td>
                    <td className="px-5 py-3">
                      <TravelSubsActions
        ...
        {pages > 1 && (
          <div className="flex justify-center gap-2 px-5 py-4 border-t border-slate-100">
            {page > 1 && (
              <Link
                href={`/admin/travel-subscriptions?status=${statusFilter}&page=${page - 1}`}
