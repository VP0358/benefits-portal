  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex gap-3">
        <a href={`/api/admin/export/orders?from=${encodeURIComponent(from)}&to=${encodeURIComponent(to)}`} className="rounded-xl border px-4 py-3 text-sm">注文CSV出力</a>
        <a href="/api/admin/export/audit" className="rounded-xl border px-4 py-3 text-sm">監査ログCSV出力</a>
      </div>
    </div>
  );
}
...
function AdminDashboardPanel({ from, to }: { from: string; to: string }) {
  ...
  if (!data || loading) {
    ...
    return <div className="rounded-3xl bg-white p-6 shadow-sm text-slate-500">読み込み中...</div>;
  }
          ...
          <div className="flex flex-wrap gap-3">
            {data.orderStatusBreakdown.map(item => (
              <div key={item.status} className="rounded-xl bg-slate-50 px-4 py-2 text-sm">
                <span className="text-slate-500">{item.status}: </span><span className="font-bold text-slate-800">{item.count}</span>
              </div>
            ))}
          </div>
        </div>
...
function MonthlyCharts({ from, to }: { from: string; to: string }) {
  ...
  if (!data || loading) {
    ...
    return <div className="rounded-3xl bg-white p-6 shadow-sm text-slate-500">グラフ読み込み中...</div>;
  }
      ...
      <div className="flex items-end gap-2 h-32">
        {data.monthly.map(row => (
          <div key={row.month} className="flex-1 flex flex-col items-center gap-1">
            ...
            <div className="text-xs text-slate-500 truncate w-full text-center">{row.month.slice(5)}</div>
          </div>
        ))}
      </div>
    </div>
...
function ProductRanking({ from, to }: { from: string; to: string }) {
  ...
  if (!data || loading) {
    ...
    return <div className="rounded-3xl bg-white p-6 shadow-sm text-slate-500">ランキング読み込み中...</div>;
  }
  return (
    <div className="rounded-3xl bg-white p-6 shadow-sm">
      <h2 className="text-lg font-bold text-slate-800 mb-4">商品別売上ランキング</h2>
      {data.ranking.length === 0 ? <div className="text-sm text-slate-500">データなし</div> : (
        <div className="space-y-2">
          {data.ranking.map(row => (
            <div key={row.rank} className="flex items-center gap-4 rounded-xl bg-slate-50 px-4 py-3 text-sm">
              <div className="text-xl font-bold text-slate-400 w-8">#{row.rank}</div>
              <div className="flex-1 font-semibold text-slate-800">{row.productName}</div>
              <div className="text-slate-600">{row.totalQuantity}個</div>
