            ...
            <div className="space-y-3">
              ...
              <div className="flex gap-2">
                ...
                <button onClick={() => setEditingId(null)}
                  className="rounded-xl border px-4 py-2 text-xs text-slate-600">キャンセル</button>
              </div>
            </div>
          ) : (
            <div>
              <div className="flex items-start justify-between gap-2">
                <div>
                  ...
                  <div className="text-xs text-slate-500 mt-0.5">契約番号: {c.contractNumber}</div>
                  <div className="text-sm text-slate-700 mt-1">月額: {c.monthlyFee.toLocaleString()}円</div>
                </div>
                <span className={`rounded-full px-2 py-1 text-xs ${statusColor[c.status] || "bg-slate-100 text-slate-500"}`}>
