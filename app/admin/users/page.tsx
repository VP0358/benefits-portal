  ...
  return (
    <main className="space-y-6">
      ...
      <div className="rounded-3xl bg-white shadow-sm overflow-hidden">
        ...
        {users.length === 0 && <div className="px-6 py-8 text-center text-slate-700 text-sm">会員がいません</div>}
        {users.map(user => (
          <div key={user.id.toString()} className="grid grid-cols-[120px_1fr_1fr_80px_120px_80px] gap-4 border-b px-6 py-4 text-sm hover:bg-slate-50">
            <div className="text-slate-800">{user.memberCode}</div>
            <div className="font-medium text-slate-800">{user.name}</div>
            <div className="truncate text-slate-800">{user.email}</div>
            <div className="text-slate-800">{user.referrals.length}</div>
            <div className="font-medium text-slate-800">{(user.pointWallet?.availablePointsBalance ?? 0).toLocaleString()}</div>
            <div>
              <Link href={`/admin/users/${user.id.toString()}`} className="rounded-lg border px-3 py-1.5 text-xs font-medium text-slate-800 hover:bg-slate-50">詳細</Link>
            </div>
          </div>
        ))}
      </div>
