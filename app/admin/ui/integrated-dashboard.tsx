...
const TAB_CONFIG: { id: Tab; label: string; icon: string; accent: string }[] = [
  ...
  { id: "mobile", label: "携帯契約",   icon: "fas fa-mobile-alt", accent: "#34d399" },
  { id: "travel", label: "格安旅行",   icon: "fas fa-plane",      accent: "#a78bfa" },
  { id: "export", label: "データ出力", icon: "fas fa-download",   accent: "#fbbf24" },
  { id: "other",  label: "設定",       icon: "fas fa-cog",        accent: "#94a3b8" },
];
  ...
  return (
    <div className="space-y-6">
      ...
      >
        ...
        {activeTab === "export" && <ExportContent accent={tab.accent} />}
        {activeTab === "other"  && <OtherContent  accent={tab.accent} />}
      </div>
    </div>
  );
}
...
function MobileContent({ accent }: { accent: string }) {
  return (
    <div>
      <SectionTitle title="携帯契約管理" subtitle="VP未来phone申し込み・契約・紹介報酬" />
      <div className="grid gap-2.5 md:grid-cols-2 lg:grid-cols-3">
        ...
        ].map(item => <NavCard key={item.href} item={item} accent={accent} />)}
      </div>
    </div>
  );
}

function TravelContent({ accent }: { accent: string }) {
  return (
    <div>
      <SectionTitle title="格安旅行管理" subtitle="旅行サブスクリプション契約の管理" />
...
function ExportContent({ accent }: { accent: string }) {
  return (
    <div>
      <SectionTitle title="データ出力" subtitle="各種CSVエクスポート・レポート出力" />
      <div className="grid gap-2.5 md:grid-cols-2">
        {[
...
function OtherContent({ accent }: { accent: string }) {
  return (
    <div className="space-y-7">
