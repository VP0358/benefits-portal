...
### ✅ 印刷・PDF機能（2026年4月10日更新）
- **登録完了通知書**: `/admin/print/registration-complete?memberId=xxx`
  - A4縦向き、日本語フォント対応
  - 見本通りレイアウト（テーブル形式）、ブラウザ印刷ダイアログ自動起動
  - 会員情報・口座情報・マイページ情報・会社フッター
- **納品書**: `/admin/print/invoice?orderId=xxx`
  - A4縦向き、税計算（8%/10%）自動計算
  - 商品明細テーブル、合計金額、交付No.表示
  - 会社登録番号付きフッター

### ✅ 会員ダッシュボード（2026年4月9日更新）
- **4種類ポイント表示**:
  - VP当月ポイント（青色プログレスバー）
  - VP先月ポイント（オレンジ色プログレスバー）
  - SAV貯金ボーナス（緑色プログレスバー）
  - MPI携帯紹介ポイント（紫色プログレスバー）

### ✅ 商品マスター管理（2026年4月10日更新）
- **商品CRUD**: 一覧表示、追加、編集、削除
- **自動計算**: 税込価格（10%）、ポイント表示（1pt = ¥100）
- **バリデーション**: 商品コード重複チェック、必須項目確認
- **商品情報**: 商品コード、商品名、説明、価格（税抜）、原価、PV、ステータス
- **ポイント付与判定**: 商品コード1000〜2999のみポイント付与対象（一覧バッジ表示・入力時リアルタイム判定）

### ✅ ボーナス設定管理（2026年4月9日更新）
- **貯金ボーナス設定**: 登録時20%, オートシップ5%, 月次コミッション3%
- **ダイレクトボーナス設定**: 直接紹介 ¥2,000/人
- **ユニレベルボーナス設定（レベル別）**:
  - **LV.1（3段目まで）**: 15%, 7%, 3%
  - **LV.2（5段目まで）**: 15%, 7%, 3%, 1%, 1%
  - **LV.3（7段目まで）**: 15%, 8%, 5%, 4%, 2%, 1%, 1%
  - **LV.4（7段目まで）**: 15%, 9%, 6%, 5%, 3%, 2%, 1%
  - **LV.5（7段目まで）**: 15%, 10%, 7%, 6%, 4%, 3%, 2%
- **組織構築ボーナス設定（レベル別）**:
  - **LV.3**: 3%
  - **LV.4**: 3.5%
  - **LV.5**: 4%
- **その他設定**: アクティブ判定pt、事務手数料、最低振込額
- **UI改善**: レベルごとに色分け（青/緑/黄/紫/赤）、見やすい入力欄

### ✅ 月次バッチ処理エンジン（2026年4月9日新規実装）
- **ボーナス計算エンジン**: 全会員の自動計算
  - 翠彩流通個数ベースのレベル判定
  - グループポイント・直接紹介アクティブ数の集計
  - ユニレベルボーナス（段数別ポイント × レベル別報酬率）
  - 組織構築ボーナス（最小系列ポイント × レベル別率）
  - ダイレクトボーナス（s1000購入個数 × ¥2,000）
  - 貯金ボーナス（月次コミッション3%）
  - 支払調整・源泉徴収・事務手数料の自動計算
  - BonusResult一括登録
- **会員レベル自動更新**: ボーナス計算完了後、currentLevelを自動更新
  - レベルアップ・レベルダウンの自動判定
  - 昇格者・降格者数の集計とログ出力
- **計算結果詳細表示**: ユニレベル段数別内訳、SAVポイント付与実績

### ✅ ボーナス計算ロジック（2026年4月9日新規実装）
- **ユニレベルボーナス**:
  - レベル別獲得段数制限（LV.1→3段、LV.2→5段、LV.3-5→7段）
  - 翠彩流通個数ベースのレベル判定（2～30個→LV.1、31～100個→LV.2、等）
  - 直接紹介2人以上購入条件
  - レベル別報酬率テーブル（仕様書2026年3月4日版準拠）
- **組織構築ボーナス（Legacy Bonus）**:
  - LV.3以上が対象、段数無制限
  - 最小系列売上から規定%付与（LV.3→3%, LV.4→3.5%, LV.5→4%）
- **直接ボーナス**: 直接紹介1人あたり ¥2,000
- **貯金ボーナス（SAVpt）**:
  - 登録時: 自己購入ptの20%
  - オートシップ決済時: 伝票合計ptの5%
  - 月次コミッション: 全ボーナス合計の3%
  - 換金: 10,000pt単位（オートシップ契約者のみ）
- **レベル達成条件**:
  - LV.1: 系列2、自己購入1個以上
  - LV.2: 系列2、自己1個、LV.1達成者各系列1名以上
  - LV.3: 系列3、自己2個、LV.1達成者各系列1名以上
  - LV.4: 系列3、自己3個、LV.2達成者各系列1名以上
  - LV.5: 系列3、自己3個、LV.3達成者各系列1名以上
...
- **MLM組織図・リスト**: 
  ...
  - ツリー表示（最大5階層、循環参照対策）
  - **レベル別色分け**: LV.0=グレー、LV.1=青、LV.2=緑、LV.3=黄、LV.4=紫、LV.5=赤
...
- **受注・発送状況管理（2026年4月10日強化）**:
  - 検索フィルター（注文日範囲、会員コード、ステータス、配送方法、**支払方法**、**伝票種別**、商品検索）
  - **支払方法**: カード、口座振替、銀行振り込み、代引き、その他
  - **伝票種別**: オートシップ、都度払い、新規、クーリングオフ、返品
  - 検索ボタンで絞り込み後に一覧表示（行クリックで詳細展開）
  - 支払方法・伝票種別バッジ表示
...
- **オートシップ管理（2026年4月9日強化）**:
  - 決済完了時SAVボーナス自動付与（伝票合計ptの5%）
  - PointWallet自動反映（externalPointsBalance）
  - 付与ログ出力
- **ボーナス計算処理**: 
  - 調整金・過不足金CRUD（手動入力・Excel一括アップロード）
  - 貯金ボーナス設定編集（登録時・オートシップ・ボーナス率）
  - ボーナス計算実行・削除機能（月次バッチエンジン統合）
- **ボーナス計算結果**:
